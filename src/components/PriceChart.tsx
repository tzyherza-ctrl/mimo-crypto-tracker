...
const ChartArea = dynamic(() => import("./RechartArea"), {
  ...
  loading: () => (
    <div className="h-64 flex items-center justify-center">
      <div className="w-5 h-5 border-2 border-[#3f3f46] border-t-white rounded-full animate-spin" />
    </div>
  ),
});
...
export default function PriceChart({ coinId }: { coinId: string }) {
  ...
  if (!data.length)
    return (
      <div className="h-64 flex items-center justify-center text-[#52525b] text-sm">
        No data
      </div>
    );
